...
function executeSql (client, targetCreds, e, callback) {
  ...
  const diSchema = identifier(`${targetCreds.schema}#DI`);
  client.exec(`SELECT TOP 1 * from ${diSchema}.M_JOBS ORDER BY JOB_START_TIMESTAMP_UTC DESC`, (err, r) => {
    if (err) {
      client.disconnect();
    ...
    if (r.length > 0) {
      ...
      const diSchema = identifier(`${targetCreds.schema}#DI`);
      client.exec(`SELECT * from ${diSchema}.M_MESSAGES where REQUEST_ID = '${latest_request_id}' AND TYPE = 'SUMMARY'`, (error, rows) => {
        if (error) {
          client.disconnect();
