  ...
  for (let i = 0; i < ref.length; i++) {
    ...
    if (incompleteKeys) {
      ...
      if (current.params && current.kind === 'entity') {
        ...
        if (!_navigation_allowed(target, current._target)) {
          _doesNotExistError(false, current.name, target.name.replace(namespace + '.', ''))
        }
...
const _navigation_allowed = (base, target) => {
  const target_service = _get_service_of(target)
  if (!target_service) return false //> target is not exposed
  if (cds.env.effective.odata.xrefs) return true //> cross-service navigation allowed
  return target_service === _get_service_of(base)
}
  ...
  for (const x of xpr) {
    ...
    if (x.ref) {
      ...
      if (x.expand) {
        ...
        if (!element._target || !_navigation_allowed(target, element._target)) {
          const target_service = _get_service_of(target)
          _doesNotExistError(
            true,
            refName,
