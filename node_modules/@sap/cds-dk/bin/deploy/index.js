module.exports = Object.assign(deploy, {
  options: ['--to', '--for', '--out', '--on', '--tunnel-address', '--vcap-file', '--delta-from' ],
...
async function deploy(_model, options) {
  ...
  try {
    ...
    const isVersionLess8 = Number(cds.version.split('.')[0]) < 8
    const localizedViewsSetting = cds.env?.sql?.transitive_localized_views
    if (isVersionLess8 && !localizedViewsSetting ) {
      cds.env.sql ??= {}
      cds.env.sql.transitive_localized_views = true
      console.warn(`transitive_localized_views = false requires @sap/cds >= 8.0.0, current version is ${cds.version}. transitive_localized_views has been set to true for this run. Will continue accordingly.`)
    }
