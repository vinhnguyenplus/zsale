...
module.exports = class JobsService extends cds.ApplicationService {
  ...
  async enqueue(service, op, clusters, args, onJobDone) {
    ...
    const job_ID = uuid()
    const job = { ID: job_ID, service, op, status: QUEUED }
    const clustered = Object.entries(clusters).map(([database, cluster]) => {
      if (database !== '0') return Array.from(cluster).map(tenant => ({ job_ID, ID: uuid(), tenant, service, op, database, status: QUEUED }))
      else return Array.from(cluster).map(tenant => ({ job_ID, ID: uuid(), tenant, service, op, status: QUEUED }))
    })
  ...
  async _nextTask(task, _fn) {
    const { ID, tenant, job_ID, op, service } = task
    try {
      await _fn
      await t0_(async () => {
        await UPDATE(Tasks, { ID, tenant }).with({ status: FINISHED })
        await this.emit('taskUpdate', { task: { ID, job_ID, tenant, op, service, status: FINISHED }})
      })
    } catch (e) {
      LOG.error(e)
      ...
      await t0_(async () => {
        ...
        await this.emit('taskUpdate', { task: { ID, job_ID, tenant, op, service, status: FAILED, error }})
      })
    }
  }
}
