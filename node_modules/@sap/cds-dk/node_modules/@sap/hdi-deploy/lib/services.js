...
const {isArray, hasTag, isUsableUserProvidedService, extractServiceData, mask_passwords_and_certificates_in_vcaps, getFilteredServices} = require('./utils');
 ...
 */
function ServiceAccessor (traceVcap) {
  ...
  this.services = xsenv.filterServices(function (service) {
    return hasTag(service, 'hana') || isUsableUserProvidedService(service);
  }).map(function (service) {
    // map HANA native tenant credentials to hdi-shared credentials
    if (service.credentials) {
      const credentials = service.credentials;
      if (!credentials.hdi_user && credentials.hdiUser) {
        credentials.hdi_user = credentials.hdiUser;
      }
      if (!credentials.hdi_password && credentials.hdiPassword) {
        credentials.hdi_password = credentials.hdiPassword;
      }
      if (!credentials.database_id && credentials.serviceInstanceID) {
        credentials.database_id = credentials.serviceInstanceID;
      }
      if (!credentials.tenant_id && credentials.id) {
        credentials.tenant_id = credentials.id;
      }
    }
    return service;
  });
  ...
  if (traceVcap) {
    try {
      ...
      for (const service of this.services) {
        if (service.label === 'hana' || service.label === 'user-provided') {
          if (!output[service.label]) {
            output[service.label] = [];
          }
          output[service.label].push(extractServiceData(service));
        }
      }
      ...
      if (typeof process.env.VCAP_SERVICES !== 'undefined') {
        ...
        const filteredServices = getFilteredServices(vcap);
        const maskedServices = mask_passwords_and_certificates_in_vcaps(
          JSON.stringify(filteredServices, null, 2)
        );
        logger.trace('VCAP_SERVICES (env):');
        logger.trace(maskedServices);
      } else {
        logger.trace('VCAP_SERVICES (env): undefined');
      }
    } catch (error) {
      logger.error(`Encountered an error while printing VCAP_SERVICES: ${error.message}`);
    }
  }
}

module.exports = function (traceVcap) {
  return new ServiceAccessor(traceVcap);
};
