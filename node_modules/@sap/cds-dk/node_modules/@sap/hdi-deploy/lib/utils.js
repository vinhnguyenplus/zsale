...
function extractMutualAuthCertificates (certs) {
  if (!certs) {
    return {};
  }
  let key = '';
  ...
  certs.split(startDelimiter)
    .filter(ctr => !!ctr)
    .map(ctr => `${startDelimiter} ${ctr.replace(/\\n/g, '').trim()}`)
    .forEach(ctr => {
      if (ctr.includes(`${startDelimiter} CERTIFICATE`)) {
        cert += ctr;
        cert += '\n';
      } else if (ctr.includes(`${startDelimiter} PRIVATE KEY`)) {
        key = ctr;
      }
    });
  return {
 ...
 */
function prepareCredentials (creds, options, logger = log) {
  ...
  if (!hdiCreds.user && hdiCreds.password) {
    const data = extractMutualAuthCertificates(hdiCreds.password);
    if (data.key) {
      logger.trace('hdiCreds.key set to private key extracted from password');
    }
    if (data.cert) {
      logger.trace('hdiCreds.cert set to certificate extracted from password');
    }
    Object.assign(hdiCreds, data);
  }

  if (creds.client_authentication_private_key) {
    hdiCreds.key = creds.client_authentication_private_key;
    logger.trace('hdiCreds.key set to client_authentication_private_key');
  }

  if (creds.client_authentication_certificate) {
    hdiCreds.cert = creds.client_authentication_certificate;
    logger.trace('hdiCreds.cert set to client_authentication_certificate');
  }

  if (hdiCreds.key && hdiCreds.cert) {
    logger.trace('hdiCreds.authenticationX509 set to key + cert');
    // authenticationX509 is the primary mutual auth mechanism. If it is not set there may be issues with hdb client.
    hdiCreds.authenticationX509 = `${hdiCreds.key}\n${hdiCreds.cert}`;
  }
