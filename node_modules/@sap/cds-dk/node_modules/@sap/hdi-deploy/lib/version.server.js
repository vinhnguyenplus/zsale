...
function getVersion (creds, cb) {
  const credentials = utils.prepareCredentials(creds, {});
  const other_parameters_in_credentials =  utils.removeManagedCredentials(creds);
  ...
  const client = hana_helper(
    ...
    credentials.key,
    credentials.cert,
    credentials.authenticationX509,
